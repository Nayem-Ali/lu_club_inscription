...
addClubInfo(Map<String, String> info) async {
  ...
  await fireStore.collection("admin").doc(auth.currentUser!.uid).update({
    "hasClub": true,
...
updatePhoto(File image, String location, String urlName, String fileName) async {
  ...
  FirebaseFirestore fireStore = FirebaseFirestore.instance;
  await fireStore.collection('clubs').doc(location).update({
    fileName: url
  });
}
...
addEvent(List<TextEditingController> eventName, List<TextEditingController> eventDescription,

    List<TextEditingController> payments, List<TextEditingController> fees, List<TextEditingController> receiverName) async {
  dynamic clubID = await clubAcronym();
  // print(clubID);
  for(int i=0; i<eventName.length; i++){
    await fireStore.collection("clubs").doc(clubID).collection("allEvents").doc(eventName[i].text
        .trim()).set(
        {
...
updateEvent(dynamic data) async {
  dynamic clubID = await clubAcronym();
  await fireStore.collection("clubs").doc(clubID).collection("allEvents").doc(data["eventName"])
      .update(data);
}
